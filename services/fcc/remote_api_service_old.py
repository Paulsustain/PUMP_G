...
REMOTE_API_URL = 'http://18.190.149.103:8000/api/v1/'
...
def get_smartpump_device_config():
    MAC = helper.get_device_mac_address()
    r = requests.get(REMOTE_API_URL+'smartpump/remote_config/', data={"mac_address": MAC})
    ...
    print(r.status_code)
    if(r.status_code == 200):
        lenght= len(r.json()['data'])
        '''for each in r.json()['data']:
            print({'init_price':each.get('First_initial_price')
             
                })
            print({'id':each.get('id')
             
                })
            
        print('This lenght is,', lenght)'''
        return r.json()['data']
...
if __name__=="__main__":
    ...
    #get_smartpump_device_config()
    print(get_smartpump_device_config())
