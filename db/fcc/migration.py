...
c.execute('''CREATE TABLE IF NOT EXISTS running_transaction
             (id integer primary key AUTOINCREMENT,
             nozzle_address int default 1,
             transaction_ongoing int default 0,
             read_at VARCHAR(50) DEFAULT NULL)''')

query = "SELECT * FROM running_transaction"

result=c.execute(query)
if(len(c.fetchall()) == 0):
    from datetime import datetime
    for number in range(1,17):
        c.execute("INSERT INTO running_transaction (nozzle_address,read_at) VALUES(?,?)", (number,datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
